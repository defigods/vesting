...
contract WalletLocking is IWalletLocking{
	...
	function withdraw(uint256 _depositId) external override {
		...
		require(vestableAmount > withdrawInfo[_depositId][address(msg.sender)], "withdraw: no tokens to withdraw at the moment");
		...
		withdrawInfo[_depositId][address(msg.sender)] = withdrawInfo[_depositId][address(msg.sender)].add(transferAmount);
	...
	function _calcVestableAmount(uint256 _depositId) public view returns (uint256) {
		...
		vestableAmount = vestableAmount + timePassed.mul(allocAmount).mul(_depositInfo._percentages[currentVestingIndex]).div(_depositInfo.vestings[currentVestingIndex]).div(100);
