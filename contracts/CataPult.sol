...
contract CataPult is ICataPult, OwnableUpgradeable{
	...
	function addTeam(address _token, uint256 _totalRaise, address _beneficiary, uint256 _startTime, uint256 _endTime, uint256 _price, uint256 _minAlloc, uint256 _maxAlloc) external onlyOwner{
	...
	function updateAllocAmounts(uint256 _teamId, uint256 _minAlloc, uint256 _maxAlloc) external onlyOwner {
	...
	function updateTimes(uint256 _teamId, uint256 _startTime, uint256 _endTime) external onlyOwner {
	...
	function withdraw(uint256 _teamId) external override {
		...
		require(_teamInfo.totalDeposit == _teamInfo.totalRaise || block.timestamp >= _teamInfo.endTime, "withdraw: not reached to limit or not ended");
